...
# In a real scenario, this would import the Core/Robot interface
...
class SafePolicy(BasePolicy):
    ...
    def __init__(self, robot_interface=None):
        self.robot = robot_interface # Injected dependency

    def execute_move(self, target_pos: Dict[str, float], context: Dict[str, Any]) -> bool:
        print(f"[SafePolicy] Moving to {target_pos} with Speed=LOW")
        # 로직:
    ...
    def execute_grasp(self, object_info: Dict[str, Any], context: Dict[str, Any]) -> bool:
        print(f"[SafePolicy] Grasping {object_info.get('name')} with vertical approach")
        # 로직:
        # 1. Z+10cm 위치에서 접근
        # 2. 천천히 하강
        # 3. 그리퍼 닫기
