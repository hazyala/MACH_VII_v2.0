...
def _identify_target_object(object_name: str, detections: list) -> dict:
    ...
    if not target and detections:
        ...
        detected_names = [d['name'] for d in detections]
        query = f"I want to catch a '{object_name}', but I only see these objects: {detected_names}. " \
                f"Which one of these is most likely the '{object_name}'? " \
                f"Return ONLY the exact name from the list. If none match, return 'NONE'."
