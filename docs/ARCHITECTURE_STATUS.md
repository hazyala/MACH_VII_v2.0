...
4.  **Strategy (L4)**: 의도를 구체적인 행동 시퀀스로 변환합니다. (검토 예정)
5.  **Expression (L5)**: 로봇의 감정 및 부드러운 움직임을 가미합니다. (검토 예정)
6.  **Embodiment (L6)**: 실물 로봇 및 PyBullet 시뮬레이터와 직접 통신하여 하드웨어를 제어합니다.
7.  **Memory (L7)**: FalkorDB를 통해 과거의 행동 결과를 그래프 데이터로 저장합니다.
